...
class TestCheckLexical(unittest.TestCase):
    ...
    def _call_check_quality(self, req):
        ...
        json_payload['config'] = { 'algorithms': ['Lexical'] }
    ...
    def test_algorithm_reached_and_found(self):
        response = self._call_check_quality('Requirement good.')
        item = self._get_single_item_from_response(response, 'Vague')
        self.assertEqual(item['title'], 'Vague')
